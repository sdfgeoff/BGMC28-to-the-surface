...
class Ship:
	def __init__(self, rootobj):
		...
		self.objs = self.rootobj.groupObject.groupMembers
	...
	def _update_particles(self):
		...
		if self._left > 0.1 or self._right > 0.1:
			# Make smoke
			down_vec = self.objs['LeftFlame'].getAxisVect([bge.logic.getRandomFloat() - 0.5, 1, 0])
			pos = (self.objs["LeftFlame"].worldPosition + self.objs["RightFlame"].worldPosition) / 2
			obj, rayhit, _nor = self.objs['LeftFlame'].rayCast(
				pos,
				pos + down_vec,
				DUST_DISTANCE,
				"", 1, False, 0,
			...
			if obj is not None:
				...
				particle = self.rootobj.scene.addObject("DustParticle", self.objs['RightFlame'], 60)
